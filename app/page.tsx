...
export default function Home() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
    setRandomTopic(""); // clear previous random topic
  ...
  const handleSurprise = () => {
    ...
    setFilteredQuotes([random.text]);
    setRandomTopic(random.topic);
    setTopic("");
    setSubmitted(true);
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-6 bg-gradient-to-br from-black via-gray-900 to-purple-900 animate-gradient-x text-white transition-colors duration-1000">
      <div className="max-w-md w-full space-y-6">
        ...
        {(topic || randomTopic) && filteredQuotes.length > 0 && (
          <p className="text-sm text-purple-300 italic text-center">
            Showing quotes for: <strong>{topic || randomTopic}</strong>
          </p>
        )}
          ...
          {filteredQuotes.length > 0 ? (
            ...
            submitted && (
              <p className="text-sm text-gray-500">
                No quotes found for &quot;{topic || randomTopic}&quot;.
              </p>
            )
          )}
        </div>
